...
# Jejaku Taste Skill: Anti-Slop Frontend Skill
...
* **NEVER use the native `<select>` element for dropdowns.** Native select popups render with OS-level UA chrome (colors, font, radius) that the page's CSS cannot fully control — on macOS/Safari this can render dark even when the rest of the page is light and `color-scheme: light` is set, breaking the design system with zero warning. Always use a custom-built dropdown component (button trigger + absolutely-positioned styled listbox, see the project's `Select` component) so every dropdown matches the app's tokens (surface color, hairline border, radius, selected-state highlight) in every browser and OS. This applies to every dropdown/select control project-wide, not just one instance.
...
**In this repo:** `/DESIGN.md` at the project root is a maintained, accurate audit of the current design language (colors, type, spacing, components, actual component names) - read it first instead of re-deriving tokens from scratch. Only fall back to a live audit (11.B) if `DESIGN.md` is missing, stale, or the brief is introducing a genuinely new surface it doesn't cover.
